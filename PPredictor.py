...
class Decoder(torch.nn.Module):
    def __init__(self, hidden_size: int, num_mlp: int):
        super().__init__()
        self.node_decoder = build_mlp(hidden_size, hidden_size, 1, num_mlp=num_mlp, lay_norm=False)

    def forward(self, x: Tensor) -> Tensor:
        predicted_poisson_ratio = self.node_decoder(x)
        return predicted_poisson_ratio
...
class Model(torch.nn.Module):
    def __init__(self, data: Data, hidden_size: int, n_layers: int, num_mlp: int, device: str = "cuda"):
        ...
        self.output_normalizer = Normalizer(size=1, name="OutputNormalizer")
        self.encoder = Encoder(data, hidden_size, num_mlp=num_mlp)
        self.gnn_layers = ModuleList()
        self.decoder = Decoder(hidden_size*4, num_mlp=num_mlp)
    ...
    def forward(self, data: Data, is_training: bool = True) -> torch.Tensor:
        ...
        # 1. Pool nodes across the N dimension -> Shape: [1, hidden_size]
        node_pool = torch.mean(data.x, dim=0, keepdim=True)

        # 2. Pool edges across the E dimension -> Shape: [1, hidden_size]
        edge_pool = torch.mean(data.edge_attr, dim=0, keepdim=True)

        # torch.max returns a tuple of (values, indices), so we grab [0]
        edge_max = torch.max(data.edge_attr, dim=0, keepdim=True)[0]

        # torch.max returns a tuple of (values, indices), so we grab [0]
        edge_min = torch.min(data.edge_attr, dim=0, keepdim=True)[0]

        # 3. Combine them -> Shape: [1, hidden_size * 2]
        graph_embedding = torch.cat([node_pool, edge_min, edge_pool, edge_max], dim=1)

        # 4. Decode to scalar -> Shape: [1, 1]
        # (Note: Your Decoder must now be sized to accept hidden_size * 2)
        model_output = self.decoder(graph_embedding)

        return model_output


    def pratio_loss(self, model_output: Tensor, target_pratio: Tensor, is_training: bool) -> Tensor:
        # Both should now be shape [1, 1] (or [Batch, 1])
        target_pratio_normalized = self.output_normalizer(target_pratio.view(-1, 1), is_training)
        return torch.nn.functional.mse_loss(model_output, target_pratio_normalized)
